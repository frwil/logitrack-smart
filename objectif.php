<?php function getTableauObjectifs()
{
    ...
    $i = 1;
    foreach ($rows as $r):
        $tableau .= "<tr><td>$i</td><td>" . h($r['date_objectif_periode']) . "</td><td>" . h($r['objectif']) . "</td><td><div class='btn-group'>".(in_array('upd',$rights_voyage) ? "<button class='btn btn-light' type='button' title='Modifier l'objectif " . h($r['date_objectif_periode']) . "' onclick='showModalUpdateObjectif(\"".sha1($r['id_objectif_periode'].$r['date_objectif_periode'])."\")'><i class='fa fa-pencil-alt'></i></button>" : "").(in_array('del',$rights_voyage) ? "<button class='btn btn-danger' title='Supprimer le objectif " . h($r['date_objectif_periode']) . "' onclick='deleteObjectif(\"".sha1($r['id_objectif_periode'].$r['date_objectif_periode'])."\")'><i class='fa fa-times'></i></button>" : "")."</div></td></tr>";
        $i++;
    endforeach;
