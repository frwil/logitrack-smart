...
class TrajetController extends BaseController
{
    ...
    public function fetchByHash(): never {
        $row = $this->repo->findByHash($this->post('id-destination-forModal'));
    ...
    public function update(): never {
        try { $this->repo->transactional(fn() =>
            $this->repo->updateByHash($this->post('id-destination'), $this->post('nom-upd-destination'), (int)$this->post('distance-destination-upd'))
        ); $this->json(); } catch (\mysqli_sql_exception $e) { $this->jsonError('Erreur'); }
    }

    public function delete(): never {
        $ok = $this->repo->deleteByHash($this->post('id-destination-forDel'));
